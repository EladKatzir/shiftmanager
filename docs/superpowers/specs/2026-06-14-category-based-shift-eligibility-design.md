...
**Date:** 2026-06-14 (design finalized 2026-06-15) · **Branch:** `dev` · **Status:** **IMPLEMENTED on `dev` 2026-06-16** (full suite 1781/1781 sequential). Plan: `docs/superpowers/plans/2026-06-16-category-based-shift-eligibility.md`. The open questions (§6) are resolved decisions (§6R).

> **Build-time resolution (with owner, 2026-06-16):** the null-`CategoryId` fork splits into **(a)** genuinely-shared shifts (HOME/OFFLINE/null-jobType) → `sharedFallback`, render all `DoesShifts` users; **(b)** an assignable shift type that simply hasn't been categorized → `noCategory`, **empty + "ask admin" nudge** PLUS a **"Show all shift workers" escape-hatch button** (re-fetches with `allowFallback=true` → promotes to the all-`DoesShifts` list). This keeps the picker from re-exposing the endless list for a real shift, while never hard-blocking the assigner. The whole switch ships behind the per-company `CategoryBasedShiftEligibility` flag (default OFF), gated a SECOND time in the UI via `CalendarPageConfig.categoryEligibilityEnabled` so flag-off is byte-identical to today for both the bottom-sheet (workforce) and quick-entry. Router = `ShiftCandidateService`; the two leaf methods gained a `categoryFilter` param (not merged). Backfill = `BackfillShiftTypeCategoryId` (+ `ShiftTypeCategoryBackfillSql`).

> **Owner correction (2026-06-16, found during the browser sweep):** HOME/OFFLINE are **presence statuses, not shifts** — a user with `DoesShifts=false` can still be marked home/offline. So `ShiftType.IsHome || IsOffline` now short-circuits (in the router, before the category/DoesShifts logic) to **everyone active non-GroupUser in the molecule**, no DoesShifts/category/rank gate. Verified live: HOME went from 1 candidate (wrong) → 33 (everyone, GroupUser excluded), while a real shift (MORNING) stays `noCategory`.

> **Browser sweep DONE (2026-06-16):** live authenticated endpoint probes confirmed the flag flips behavior (legacy 20/34 → category-rule), the null-category fork (noCategory/sharedFallback/escape-hatch), category-member filtering (via a reverted fixture), and the HOME fix; **zero console errors**; the calendar renders in light/dark/RTL; and the quick-entry dropdown states (two-line user rows, conflict accent, no-category nudge + "Show all" button, loading/empty) were visually verified in light/dark/RTL via a faithful CSS harness. **Remaining before per-company enable:** FinalProductPublish regen (needs a release `-Version`).
