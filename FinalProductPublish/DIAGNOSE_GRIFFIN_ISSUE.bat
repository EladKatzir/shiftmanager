...
powershell -NoProfile -Command "$db = 'app.db'; $query = 'SELECT Id, CompanyId, Enabled, BaseUrl, TokenConsumerUrl, TimeoutSeconds FROM GriffinConfigs'; try { Add-Type -Path 'System.Data.SQLite.dll' -ErrorAction Stop; $conn = New-Object System.Data.SQLite.SQLiteConnection(\"Data Source=$db\"); $conn.Open(); $cmd = $conn.CreateCommand(); $cmd.CommandText = $query; $reader = $cmd.ExecuteReader(); Write-Host 'GriffinConfig Records:'; Write-Host '----------------------------------------'; $found = $false; while ($reader.Read()) { $found = $true; Write-Host ('ID: ' + $reader['Id']); Write-Host ('CompanyId: ' + $reader['CompanyId']); Write-Host ('Enabled: ' + $reader['Enabled']); Write-Host ('BaseUrl: ' + $reader['BaseUrl']); Write-Host ('TokenConsumerUrl: ' + $reader['TokenConsumerUrl']); Write-Host ('TimeoutSeconds: ' + $reader['TimeoutSeconds']); Write-Host '----------------------------------------'; } $reader.Close(); $conn.Close(); if (-not $found) { Write-Host 'No GriffinConfig records found!'; Write-Host 'You need to configure Griffin ADFS at http://localhost:5000/Owner/GriffinConfig'; } } catch { Write-Host 'ERROR: Could not query database using .NET SQLite'; Write-Host $_.Exception.Message; Write-Host ''; Write-Host 'Trying alternative method with sqlite3.exe...'; }" 2>nul
