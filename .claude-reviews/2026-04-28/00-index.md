...
- Last updated: 2026-05-08 (Batch K Phase 1+2+3 — Login.cshtml.cs 43 + NotificationService.cs 39 + SwapRequestsController.cs 42 = 124 LoggerMessage source-gen sites converted; build 2,524 → **2,400 warnings, 0 errors** [-124], tests **1,196/1,196 passing**)
...
| **K — Hot-path perf sweep** | ✅ Phase 1+2+3 / Continuing | **Phase 1** (`Pages/Auth/Login.cshtml.cs`, 43 sites, EventIds 1000-1055): build 2,524→2,481 [-43]. **Phase 2** (`Services/NotificationService.cs`, 39 sites, EventIds 2000-2064): build 2,481→2,442 [-39]. Surfaced design refinement: nullable-bearing templates need matching `int?` signatures (catch blocks where companyId may be unset). **Phase 3** (`Controllers/Api/V1/SwapRequestsController.cs`, 42 sites, EventIds 3000-3071): build 2,442→**2,400** [-42]. Demonstrated **method reuse pattern** — 21 partial methods cover 42 sites because boilerplate events (endpoint-disabled / unauthorized / DB-error catch) fire identically across 6 endpoints, distinguished by `Endpoint` parameter. Also handled mixed `int companyId` (success path) vs `string? companyId` (catch with raw claim) for the same template literal. **Cumulative -124 warnings** with build clean and **1,196/1,196 tests passing**. **Remaining Phase 4+**: ~1,600 sites in Calendar/Table (39), Admin/Users (60), MailService, GriffinConfigService, My/Requests, plus 158 `.Any()` + 29 `JsonOptions`. Per-file PRs only. |
