...
- Last updated: 2026-05-08 (Batch K Phase 1+2 — Login.cshtml.cs 43 sites + NotificationService.cs 39 sites = 82 LoggerMessage source-gen sites converted; build 2,524 → **2,442 warnings, 0 errors** [-82], tests **1,196/1,196 passing**)
...
| **I — Task.Run cleanup** | ⏸️ Deferred | F-A-003 sites all already wrap `try/catch` inside the lambda; what's missing is observability/back-pressure infrastructure (`IBackgroundTaskQueue`). Proper fix is infrastructure, not inline. |
...
| **K — Hot-path perf sweep** | ✅ Phase 1+2 / Continuing | **2026-05-08 Phase 1**: `Pages/Auth/Login.cshtml.cs` LoggerMessage source-gen (43 sites). EventIds 1000-1055 in new `Pages/Auth/Login.cshtml.Logging.cs`. CA1848 in file 43→**0**, build 2,524 → 2,481 [-43]. **2026-05-08 Phase 2**: `Services/NotificationService.cs` LoggerMessage source-gen (39 sites). EventIds 2000-2064 in new `Services/NotificationService.Logging.cs` (sub-blocks: 2000-2009 Create core, 2010-2019 email fan-out, 2020-2029 access request, 2030-2039 daily digest, 2040-2049 day-before reminders, 2050-2059 Ops Console, 2060-2069 TryCreate diagnostics). CA1848 in file 39→**0**, build 2,481 → **2,442** [-39]. **Cumulative -82 warnings** with build clean and **1,196/1,196 tests passing**. Phase 2 surfaced one design refinement: nullable-bearing log templates (e.g., `int? companyId` in catch blocks) require matching nullable signatures in partial methods — recorded in CLAUDE.md memory for Phase 3+ files. **Remaining Phase 3-5**: ~1,640 sites in SwapRequestsController (42), Calendar/Table (39), Admin/Users (60), MailService, GriffinConfigService, My/Requests, etc. + 158 `.Any()` + 29 `JsonOptions`. Per-file PRs only (handoff §6 anti-pattern #6). |
