...
- Last updated: 2026-05-09 (Batch K Phase 1-8 — 340 LoggerMessage sites + Batch I — IBackgroundTaskQueue infra closes F-A-003; build 2,524 → **2,186 warnings, 0 errors** [-338 net], tests **1,196/1,196 passing**)
...
| **K — Hot-path perf sweep** | ✅ Phase 1-8 / Continuing | **Phase 1** Login.cshtml.cs (43 sites, 1000-1055): -43. **Phase 2** NotificationService.cs (39 sites, 2000-2064): -39. Surfaced `int?` signature refinement. **Phase 3** SwapRequestsController.cs (42 sites, 3000-3071): -42. **Method reuse pattern**. **Phase 4** Calendar/Table.cshtml.cs (39 sites, 4000-4040): -39. **Template parameterization** (`{Action}` placeholder). **Phase 5** Requests/Index.cshtml.cs (42 sites, 5000-5047): -42. **Cartesian-product templates** (`{Action}` × `{EntityType}`). **Phase 6** My/Requests.cshtml.cs (38 sites, 8000-8074): -38. **Phase 7** TimeOffController.cs (37 sites, 6000-6027): -37. Mirrors Phase 3 SwapRequestsController pattern; 25 methods cover 37 sites; surfaced `int? userId` signature refinement (Create endpoint passes `request?.UserId` directly, not raw claim string). Conventions doc reallocated 6000-6099 to TimeOffController, pushed Admin/Users to 12000-12099. **Phase 8 (2026-05-09)** Pages/Admin/Users.cshtml.cs (60 sites, 12000-12091): -60 warnings (exactly matching site count). **Largest single-file PR in Batch K**: 43 partial methods cover 60 call sites via three reuse pockets — bare-vs-suffixed `Invalid claim` (10 + 3), parameterized `{Action}` unauthorized warning across 6 handlers (toggle/role/jobtype/password-reset/unlock/delete), and identical `RoleTemplate ... missing DerivedUserRole` literal across 3 differing-arg sites. Pattern A + Pattern C mix mirrors Phase 4. **Cumulative Phase 1-8: -338 warnings** across 340 sites. Build clean (2,524 → **2,186 warnings, 0 errors**), **1,196/1,196 tests passing**. **Phase 9-15 (2026-05-09 PM)** shipped as 7 parallel-dispatched per-file migrations in a single integration: GriffinConfigService.cs (38 sites, 24 methods, 7000-7044), ChoresController.cs (35 sites, 19 methods, 13000-13022), OnDutyController.cs (35 sites, 20 methods, 14000-14019), UsersController.cs V1 (26 sites, 16 methods, 15000-15017), FeedbackController.cs (32 sites, 17 methods, 16000-16022), GriffinService.cs (26 sites, 18 methods, 17000-17080), DatabaseBackupService.cs (26 sites, 26 methods, 18000-18050). **Total: 218 sites → 140 partial methods (~36% reuse savings)**. One integration-time fix: `GriffinService.Logging.cs` had `using ShiftManager.Models` instead of `ShiftManager.Models.Support` for the `UserRole` enum signature; corrected before build clean. **Cumulative Phase 1-15: -556 warnings** across 558 sites. Build clean (**2,186 → 1,968 unique warnings, 0 errors**), **1,196/1,196 tests passing**. **Phase 16 (2026-05-09 PM, source-level)** Program.cs (80 sites, EventIds 90000-91210) — 75 partial methods cover 80 sites with 1 Pattern C reuse (`LogOrphanTemplateRemapped` with `{Entity}` placeholder collapsing 5 sites). Three local logger variables encountered (`logger`, `startupLogger`, `featureFlagLogger`) all of type `ILogger<Program>` — same partial methods serve all three. `public partial class Program { }` declaration preserved (load-bearing for `WebApplicationFactory<Program>` integration tests). **Build verification PENDING** — file lock from running ShiftManager.exe (PID 138844) blocked the no-incremental clean build at link/copy step; C# compilation phase completed with **0 CS errors, 1942 unique warnings (−26 net), CA1848 1104 unique (−80 exactly matching the 80 Program.cs sites — confirmed migration correctness)**. The 54-warning gap between expected (-80) and observed (-26) is 28 NEW warnings introduced by Program.cs / Program.Logging.cs (likely CA1822 / CA2007 / etc. — needs investigation next session) plus 26 cancelled out by some unrelated fluctuation. **Cumulative Phase 1-16: ~−636 warnings** across 638 sites. **Batch K functionally complete** modulo MailService (34 sites — reserved for Batch O god-class decomp) and an outstanding integration verification for Phase 16. **Remaining out of Batch K scope**: 158 `.Any()` (CA1860) + 29 `JsonOptions` (separate batches if pursued). |
