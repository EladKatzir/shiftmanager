...
- Last updated: 2026-05-08 (Batch K Phase 1 — Login.cshtml.cs LoggerMessage source-gen, 43 sites; build 2,524 → **2,481 warnings, 0 errors** [-43], tests **1,196/1,196 passing**)
...
| **K — Hot-path perf sweep** | ✅ Phase 1 / Continuing | **2026-05-08 Phase 1 shipped**: `Pages/Auth/Login.cshtml.cs` LoggerMessage source-gen (43 sites, F-C-013 partial). Class made `partial`; new `Pages/Auth/Login.cshtml.Logging.cs` holds 43 `[LoggerMessage]` declarations spanning EventIds 1000-1055 (1000-1003 OnGetAsync, 1010-1022 OnPostAsync, 1030-1055 OnPostGriffinAsync; range 1000-1099 reserved for /Auth/Login). Verified: CA1848 in this file 43→**0**, total CA1848 3,484→3,398 (−86 log emissions = −43 unique sites — MSBuild prints each warning twice but the `Warning(s)` summary deduplicates). Build clean: **2,524 → 2,481 warnings, 0 errors** [-43]. Tests **1,196/1,196 passing**. Ancillary: new `docs/reference/LOGGING-CONVENTIONS.md` enumerates per-file EventId blocks (1000-1099 Login claimed, 1100+ reserved); pointer comment added to `Directory.Build.props`. **Remaining Phase 2-5**: ~1,680 sites in NotificationService (39), SwapRequestsController (42), Calendar/Table (39), Admin/Users (60), MailService, etc. + 158 `.Any()` + 29 `JsonOptions`. Per-file PRs only (handoff §6 anti-pattern #6). |
