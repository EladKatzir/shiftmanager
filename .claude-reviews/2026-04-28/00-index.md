...
- Last updated: 2026-05-09 (Batch K Phase 1-6 — 243 LoggerMessage sites + Batch I — IBackgroundTaskQueue infra closes F-A-003; build 2,524 → **2,283 warnings, 0 errors** [-241 net], tests **1,196/1,196 passing**)
...
| **K — Hot-path perf sweep** | ✅ Phase 1-6 / Continuing | **Phase 1** (`Pages/Auth/Login.cshtml.cs`, 43 sites, 1000-1055): -43. **Phase 2** (`Services/NotificationService.cs`, 39 sites, 2000-2064): -39. Surfaced `int?` signature refinement. **Phase 3** (`Controllers/Api/V1/SwapRequestsController.cs`, 42 sites, 3000-3071): -42. **Method reuse pattern** (21 methods → 42 sites). **Phase 4** (`Pages/Calendar/Table.cshtml.cs`, 39 sites, 4000-4040): -39. **Template parameterization** (14 methods → 39 sites). **Phase 5** (`Pages/Requests/Index.cshtml.cs`, 42 sites, 5000-5047): -42. **Cartesian-product templates** — `{Action}` × `{EntityType}` collapses 8 literals into 2 methods (28 methods → 42 sites). **Phase 6** (`Pages/My/Requests.cshtml.cs`, 38 sites, 8000-8074): -38. Companion to Phase 5 (employee-facing view of same domain); 35 methods cover 38 sites with claim literal reused 4x across 4 handlers. **Cumulative Phase 1-6: -243 warnings** across 243 sites. Build clean, **1,196/1,196 tests passing**. **Remaining**: ~1,477 sites in Admin/Users (60), MailService (34, Batch O scope), GriffinConfigService (38), V1 controllers TimeOff/Chores/OnDuty/Users/Feedback (~37 each), plus 158 `.Any()` + 29 `JsonOptions`. Per-file PRs only. |
