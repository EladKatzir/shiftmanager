...
- Last updated: 2026-05-09 (Batch K Phase 1-5 — 205 LoggerMessage sites + Batch I — IBackgroundTaskQueue infra closes F-A-003; build 2,524 → **2,321 warnings, 0 errors** [-203 net], tests **1,196/1,196 passing**)
...
| **K — Hot-path perf sweep** | ✅ Phase 1-5 / Continuing | **Phase 1** (`Pages/Auth/Login.cshtml.cs`, 43 sites, 1000-1055): -43. **Phase 2** (`Services/NotificationService.cs`, 39 sites, 2000-2064): -39. Surfaced `int?` signature refinement. **Phase 3** (`Controllers/Api/V1/SwapRequestsController.cs`, 42 sites, 3000-3071): -42. Established **method reuse pattern** (21 methods → 42 sites). **Phase 4** (`Pages/Calendar/Table.cshtml.cs`, 39 sites, 4000-4040): -39. Established **template parameterization** (14 methods → 39 sites; `LogErrorAction(ex, "ensuring shift instance")`). **Phase 5** (`Pages/Requests/Index.cshtml.cs`, 42 sites, 5000-5047): -42. Extended parameterization to **Cartesian-product templates** — `{Action}` × `{EntityType}` collapses 8 SECURITY+CONCURRENCY literals (approve/decline × time off/swap) into 2 partial methods. 28 methods cover 42 call sites. **Cumulative Phase 1-5: -205 warnings** across 205 sites. Build clean, **1,196/1,196 tests passing**. **Remaining**: ~1,515 sites in Admin/Users (60), MailService (34, Batch O scope), GriffinConfigService (38), My/Requests (38), V1 controllers TimeOff/Chores/OnDuty/Users/Feedback (~37 each), plus 158 `.Any()` + 29 `JsonOptions`. Per-file PRs only. |
